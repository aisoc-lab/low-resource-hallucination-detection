...
import os, json, sys, gc, argparse, time, re, logging, torch, numpy as np, pandas as pd
...
class UnigramModel:
    ...
    def add(self, text: str) -> None:
        """
        Add/Count number of unigrams in text
        """
    ...
    def evaluate(self, text: str) -> float:
        """
        Calculate the negative log likelihood of the model on the evaluation text
        """
        logprob_doc = []
        tokens = [token.text for token in self.nlp(text)]
        for token in tokens:
        ...
        return {'doc_level': {'avg_neg_logprob': avg_neg_logprob_doc}}
    
class NgramModel:
    def __init__(self, n: int, lang_code: str, left_pad_symbol: str = '<s>') -> None:
        self.nlp = spacy.blank(lang_code)
        self.ngram_count = 0
        self.counts = {'<unk>': 0}
        self.n = n
    ...
    def add(self, text: str) -> None:
        """
        Add/Count number of ngrams in text
        """
        tokens = [token.text for token in self.nlp(text)]
...
class SelfCheckNgram:
    ...
    def predict(
        ...
        if self.n == 1:
            ngram_model = UnigramModel(lang_code=lang_code)
        elif self.n > 1:
            ngram_model = NgramModel(n=self.n, lang_code=lang_code)
        else:
        ...
        ngram_pred = ngram_model.evaluate(passage)
        return ngram_pred
...
def generate_responses(**gen_res_args):
    ...
    lang_code = gen_res_args.get('lang_code')
    ...
    high_temp_str_responses = [r[0] for r in full_responses]
    sent_scores_ngram = SelfCheck_Ngram.predict(   
        passage = low_temp_response,
        sampled_passages = high_temp_str_responses,
        lang_code = lang_code
    )
    ...
    return low_temp_response, start_pos, full_responses, selfcheck_ngram_overall
...
def answer_question(**ans_ques_args):
    ...
    lang_code = ans_ques_args.get('lang_code')
    ...
    str_response, start_pos, full_responses, selfcheck_ngram_overall = generate_responses(
        question=chat_str, 
    ...
    result = {
            ...
            'selfcheck_ngram_overall': selfcheck_ngram_overall,
        }

    generations.append(result)

    return correct, selfcheck_ngram_overall, generations
...
def compute_auroc(correct_arr, uncertain_arr, selfcheck_ngram_average, output_file_path):
    try:
        auroc_uncertain = roc_auc_score(uncertain_arr, selfcheck_ngram_average)
        logging.info(f"AUROC for self check ngram (uncertain): {auroc_uncertain}")
        with open(output_file_path,'a', encoding='utf-8') as f:
            print(f"Correct Array: {correct_arr}", file=f)
            logging.info(f"Correct Array: {correct_arr}")
            print(f"Self Check Ngram Average: {selfcheck_ngram_average}", file=f)
            logging.info(f"Self Check Ngram Average: {selfcheck_ngram_average}")
            print(f"AUROC for self check ngram (uncertain): {auroc_uncertain}", file=f)
            logging.info(f"AUROC for self check ngram (uncertain): {auroc_uncertain}")
            print("="*25, file=f)
            logging.info("="*25)
    except Exception as e:
        logging.info(f"Error computing AUROC score: {e}")


def saving_scores(correct_arr, output_file_path, generations, actual_questions, results_file_path):
    correct_count = correct_arr.count(1)
...
def processing_dataset(data_category, model, tokenizer, device, args):
    results_dir = f"{args.base_results_dir}/SelfCheckGPT_{args.dataset_name}_results_v1/{args.model_name}_SelfCheckGPT_{data_category}_{args.language}_results"
    ...
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',
                    handlers=[
                    logging.FileHandler(f"{results_dir}/{args.model_name}_SelfCheckGPT_{data_category}_{args.language}_results_log.txt"),
    ...
    if args.dataset_name == "mTREx":
        ...
        for idx in tqdm(range(len(data)), desc="Processing questions"):
            try:
                ...
                correct, selfcheck_ngram_overall, generations = answer_question(
                        source=source,
                        target=target,
                        tokenizer=tokenizer,
                        model=model,
                        ...
                        mTREx_category=data_category,
                        lang=args.language,
                        lang_code=lang_codes[args.language.lower()],
    ...
    compute_auroc(correct_arr, uncertain_arr, selfcheck_ngram_average, output_file_path)
    saving_scores(correct_arr, output_file_path, generations, actual_questions, results_file_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate SelfCheckGPT method across different LLMs and languages for G-MMLU and mTREx datasets.")
